...
public final class Glace2d
{
	...
	private void iniCenter() 
	{
		...
		{
			int jId = jCells;
			Real2 sum1374354280 = new Real2(0.0, 0.0);
			...
			for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
			{
				...
				int rNodes = rId;
				sum1374354280.operator_set(sum1374354280.operator_plus((coord[rNodes])));
			}
			center[jCells].operator_set(sum1374354280.operator_multiply((1.0 / 4.0)));
		});
	}		
	...
	private void iniVIc() 
	{
		...
		{
			int jId = jCells;
			double sum752737616 = 0.0;
			int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
			for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
			{
				int rId = nodesOfCellJ[rNodesOfCellJ];
				int rNodes = rId;
				sum752737616 = sum752737616 + (MathFunctions.dot(C_ic[jCells][rNodesOfCellJ], coord[rNodes]));
			}
			V_ic[jCells] = 0.5 * sum752737616;
		});
	}		
	...
	private void computeV() 
	{
		...
		{
			int jId = jCells;
			double sum139109156 = 0.0;
			int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
			for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
			{
				int rId = nodesOfCellJ[rNodesOfCellJ];
				int rNodes = rId;
				sum139109156 = sum139109156 + (MathFunctions.dot(C[jCells][rNodesOfCellJ], X[rNodes]));
			}
			V[jCells] = 0.5 * sum139109156;
		});
	}		
	...
	private void computedeltatj() 
	{
		...
		{
			int jId = jCells;
			double sum649948385 = 0.0;
			int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
			for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
			{
				sum649948385 = sum649948385 + (l[jCells][rNodesOfCellJ]);
			}
			deltatj[jCells] = 2.0 * V[jCells] / (c[jCells] * sum649948385);
		});
	}		
	...
	private void computeAr() 
	{
		...
		{
			int rId = rNodes;
			Real2x2 sum210103322 = new Real2x2(new Real2(0.0, 0.0), new Real2(0.0, 0.0));
			...
			for (int jCellsOfNodeR=0; jCellsOfNodeR<cellsOfNodeR.length; jCellsOfNodeR++)
			{
				...
				int jCells = jId;
				sum210103322.operator_set(sum210103322.operator_plus((Ajr[jCells][rNodesOfCellJ])));
			}
			Ar[rNodes].operator_set(sum210103322);
		});
	}		
	...
	private void computeBr() 
	{
		...
		{
			int rId = rNodes;
			Real2 sum1055739971 = new Real2(0.0, 0.0);
			int[] cellsOfNodeR = mesh.getCellsOfNode(rId);
			for (int jCellsOfNodeR=0; jCellsOfNodeR<cellsOfNodeR.length; jCellsOfNodeR++)
			{
				int jId = cellsOfNodeR[jCellsOfNodeR];
				...
				int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
				int rNodesOfCellJ = Utils.indexOf(nodesOfCellJ,rId);
				sum1055739971.operator_set(sum1055739971.operator_plus((C[jCells][rNodesOfCellJ].operator_multiply(p[jCells]).operator_plus(Glace2dFunctions.matVectProduct(Ajr[jCells][rNodesOfCellJ], uj[jCells])))));
			}
			b[rNodes].operator_set(sum1055739971);
		});
	}		
	...
	{
		double reduceMin911356133 = IntStream.range(0, nbCells).boxed().parallel().reduce(
			Double.MAX_VALUE, 
			(r, jCells) -> MathFunctions.reduceMin(r, deltatj[jCells]),
			(r1, r2) -> MathFunctions.reduceMin(r1, r2)
		);
		deltat_n_plus_1 = options.option_deltat_cfl * reduceMin911356133;
	}		
	...
	private void compute_ComputeUn() 
	{
		...
		{
			int jId = jCells;
			Real2 sum1575564851 = new Real2(0.0, 0.0);
			int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
			for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
			{
				sum1575564851.operator_set(sum1575564851.operator_plus((F[jCells][rNodesOfCellJ])));
			}
			uj_n_plus_1[jCells].operator_set(uj[jCells].operator_minus(sum1575564851.operator_multiply((deltat / m[jCells]))));
		});
	}		
	...
	private void compute_ComputeEn() 
	{
		...
		{
			int jId = jCells;
			double sum2127951544 = 0.0;
			int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
			for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
			{
				int rId = nodesOfCellJ[rNodesOfCellJ];
				int rNodes = rId;
				sum2127951544 = sum2127951544 + (MathFunctions.dot(F[jCells][rNodesOfCellJ], ur[rNodes]));
			}
			E_n_plus_1[jCells] = E[jCells] - (deltat / m[jCells]) * sum2127951544;
		});
	}		
