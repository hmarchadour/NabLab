...
public class SiriusServerFrontendFilter implements Filter {
	 ...
	 * The path of the HTTP API.
	 */
	private static final String WS_DIAGRAMS_PATH = "/diagrams"; //$NON-NLS-1$
	 ...
	 * @return <code>true</code> if the given request matches a static resources,
	...
	/**
	 * Indicates if the given request URI should be redirected to the front end for
	 * a proper single page application.
	 *
	 * @param requestURI
	 *            The URI of the request
	 * @return <code>true</code> if the given request should be redirected to the
	 *         front end
	 */
	private boolean isFrontEnd(String requestURI) {
		...
		isFrontEnd = isFrontEnd && !requestURI.startsWith(WS_DIAGRAMS_PATH);
	 ...
	 *             In case of error
	 */
	private void handleStaticResource(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,
			FilterChain chain) throws IOException, ServletException {
		chain.doFilter(httpServletRequest, httpServletResponse);
	}

	/**
	 * Redirects the request to the front end in order to have a proper single page
	 * application.
	 *
	 * @param httpServletRequest
	 *            The request
	 ...
	 *             In case of error
	 */
	private void redirectToFrontEnd(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
