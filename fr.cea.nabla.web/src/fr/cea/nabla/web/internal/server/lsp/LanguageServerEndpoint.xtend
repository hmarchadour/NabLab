...
class LanguageServerEndpoint extends Endpoint {
	...
	override onOpen(Session session, EndpointConfig config) {
		...
		val outgoingMessageStream = new CustomStreamMessageConsumer(session.asyncRemote, jsonHandler)
		...
		val incomingMessageStream = new CustomStreamMessageProducer(null, jsonHandler)
